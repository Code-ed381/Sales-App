    ...
    def addNewUser(self):
        ...
        if user_role == 0:
            try:
                ...
                data = (username, fullname, email, password,self.img[0], 'user')
                mycursor.execute(query, data)
                db.commit()
            except:
                QMessageBox.critical(
        ...
        if user_role == 0:
            self.users_table.setItem(rowPosition, 4, QTableWidgetItem("user"))
        elif user_role == 1:
...
class Purchases(QMainWindow):
    def __init__(self):
        ...
        for e in rows:
            ...
            self.deletebtn.setIcon(QIcon('./icons/recycle-bin-line-icon.png'))
            self.deletebtn.setStyleSheet('QPushButton { background-color:  rgb(170, 0, 0); color: #fff }')
            ...
            self.table.setCellWidget(row, 6, self.deletebtn)
            row += 1

    def deleteClicked(self):
        button = self.sender()
        item = self.users_table.item(row, 0).text()
        ...
        if button:
            ...
            if button == QMessageBox.StandardButton.Yes:
                ...
                try:
                    try:
                        query = "DELETE FROM purchase WHERE PurchaseID = ?"
                        mycursor.execute(query, (item,))
                        db.commit()
                    except:
                        QMessageBox.critical(
